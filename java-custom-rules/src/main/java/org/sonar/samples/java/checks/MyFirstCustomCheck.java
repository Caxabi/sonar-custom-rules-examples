...
public class MyFirstCustomCheck extends IssuableSubscriptionVisitor {
	...
	public void visitNode(Tree tree) {
		...
		if (method.parameters().size() == 1) {
			MethodSymbol symbol = method.symbol(); // 2nd version
			Type firstParameterType = symbol.parameterTypes().get(0); // 2nd version
			Type returnType = symbol.returnType().type(); // 2nd version
